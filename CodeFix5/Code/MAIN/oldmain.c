...
#include "../CLUSTER/DATASTRUCT/cluster.h"
#include "../CLUSTER/FUNCTIONS/clusterfunctions.h"
...
int SiteHop(SNarray snA, Charge one, SiteNode site, int * totalX);
...
int SiteHop(SNarray snA, Charge one, SiteNode site, int * totalX){
	...
	//initially, the flag is set as "one may hop"
	int flag = 0; 
  double position;
  int low, middle, high;
	int rv;

	//if the first charge in the queue isn't at an electrode
	if(getCx(one)!= 0 && getCx(one) != SLength-1) {  
		//nor at the interfaces with substrate and air
		if(getCz(one) != 0 && getCz(one)!= SHeight-1){  
			
			rv = OccAllNeiPeriodicY(snA, getCx(one), getCy(one), getCz(one), SWidth);
			if (rv==1){
				//there is no possibility for hopping as all the neighbouring sites 
				//around the first charge position are all occuppied
				flag = 1;
			}
		}
	}
	//printf("Check all Neighbors flag %d\n",flag);
	//there is a possibility for hopping as at least one of the neighbouring 
	//site is unoccupied. this loop determines in which direction the charge 
	//will jump to by randomly choosing a number between 0 and 1 
	//(random/max_random) and comparing it to p[0],p[1],p[2],p[3],p[4],p[5] values

	assert(getp(site,5,0)<1.01);
	assert(getp(site,5,0)>0.99);
	...
	if(flag == 0){   
		...
		high = 5;
		do{
			//if random nb "position" is inf or equal p[0] of this site, the charge 
			//will go backward (x-1)
			if( getp(site,0,0)>= position){   
				//set middle to 0 (and proceed to case 0)
				//if position is less than the first pval
				middle = 0;   
				break;
			}
			...
			//if p val of 3 is greater than position has to be between 1, 2 and 3
			if(getp(site,middle, 0) >= position){
				//else, if random nb "position" is comprised between p[middle] and 
				//p[middle-1] of this site, proceed to case "middle", ie the charge will 
				//go into the "middle" direction
				//if pval 2 is less than position must be 3
				if(getp(site,middle-1,0) < position) {   
					break;
					//middle is = 3
				}else{
					//if pval 2 is greater than 3 must be 2 or 1
					//what if this gives an non integer middle value?

					//if pval 1 is greater than position but we have ruled
					//out 0 must be 1
					if(getp(site,1,0)>=position){
						middle=1;
						break;
					}else{
						middle =2;
				...
				//Means it is between 4 and 5
			}else{

				//else, if random nb "position" is sup to p[++middle], then try again to 
				//find where the charge will jump by changing lower value
				if(getp(site,4,0) > position) {   
			 		if(getCz(one) == 0) {
						middle=5;
						break;
					}else{

						if(getp(site,4,0)!=getp(site,3,0)){
							middle=4;
							break;
						}else{
							middle=3;
		...
		//loop forever until a breaking condition is reached, ie until the value of 
		...
		switch(middle){
			...
			//ie negative current
			case 0: if(getCx(one) == 0){  
								//Move the charge backward
								CxMinus(one);
								//decrease the distance run by the charge in the x direction
								XdistMinus(one);
								//current decreases
								*totalX=*totalX-1;;   
								break;
							}
							//else, if the site just behind (x-1) the charge (at x) is occupied
							else if( OccXneg(snA, getCx(one),getCy(one),getCz(one))==1) {
								//there is no possibility to hopp
								flag = 1;   
								printf("flag behind%d\n",flag);
								break;
							}
							//Move the charge backward
							CxMinus(one);
			...
			case 1: if(getCx(one) == SLength - 1){   
								//move the charge forward
								CxPlus(one);   
								//increase the distance run by the charge in the x direction
								XdistPlus(one);
								//current increases
								*totalX=*totalX+1;   
								break;
							}
							//else, if the site before (x+1) the charge position (at x) is occupied
							else if( OccXpos(snA, getCx(one),getCy(one),getCz(one))== 1){  
								//there is no possibility to hop
								flag = 1;   
								printf("flag infront%d\n",flag);
								break;
							}
							//move the charge forward
							CxPlus(one);   
			...
			case 2: if( OccYnegPeriodic(snA, getCx(one), getCy(one), getCz(one))==1){ 
								//there is no possibility to hopp
								flag = 1;
								printf("flag left%d\n",flag);
								break;
							...
							CyMinusPeriodic(one,SWidth);
							break;

			//if the site on the right of the charge position is occupied,
			case 3: if( OccYposPeriodic(snA, getCx(one),getCy(one),getCz(one))==1){   
								//there is no possibility to hopp
								flag = 1;   
								printf("flag right%d\n",flag);
								break;

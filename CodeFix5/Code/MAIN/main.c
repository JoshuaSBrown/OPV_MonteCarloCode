				...
				for(count=0;count<TemperatureStep;count++){
					...
					for(r=1;r<Rcount+1;r++){
						...
						initElec(electricEnergyX, electricEnergyY, electricEnergyZ,\
								MarcusCoeff, KT, reOrgEnergy, snA, RelativePerm,\
								SiteDistance, PeriodicX, PeriodicY, PeriodicZ,\
								XElecOn, YElecOn, ZElecOn,\
								XFermiB, YFermiL, ZFermiB, XFermiF, YFermiR, ZFermiA,\
								alphaxb, alphaxf, alphayl, alphayr, alphazb, alphaza,\
						...
						randomWalk(snA, TCount, TStep,\
								Nstep_av, NCh, D, rN,\
								XElecOn, YElecOn, ZElecOn,\
								EndX, EndY, EndZ, PeriodicX,\
								PeriodicY, PeriodicZ, &FileName[0],\
								SiteDistance, electricFieldX,\
								electricFieldY, electricFieldZ,\
								elXb, elXf, elYl, elYr,\
								elZb, elZa); 
